...
test_that("baseline hazard (no strata): compare to survival::basehaz", {
  ## vs basehaz
  expect_equal(
    ignore_attr = TRUE, predictCox(fit.coxph, centered = FALSE)$cumhazard,
    survival::basehaz(fit.coxph, centered = FALSE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fit.coxph, centered = TRUE)$cumhazard,
    survival::basehaz(fit.coxph, centered = TRUE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fit.cph)$cumhazard,
  ...
  expect_equal(
    ignore_attr = TRUE, predictCox(fit.cph, centered = FALSE),
...
test_that("baseline hazard (strata): compare to survival::basehaz", {
  ## vs basehaz
  expect_equal(
    ignore_attr = TRUE, predictCox(fitS.coxph, centered = FALSE)$cumhazard,
    basehaz(fitS.coxph, centered = FALSE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fitS.coxph, centered = TRUE)$cumhazard,
    basehaz(fitS.coxph, centered = TRUE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fitS.cph)$cumhazard,
    basehaz(fitS.cph)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fitS.coxph, keep.strata = TRUE)$strata,
    basehaz(fitS.coxph)$strata
  )
  expect_equal(
    ignore_attr = TRUE, predictCox(fitS.cph, keep.strata = TRUE)$strata,
...
test_that("baseline hazard (strata): number of events", {
  GS.alltime <- tapply(
    Melanoma$time, interaction(Melanoma$ici, Melanoma$invasion),
    function(x) {
      sort(unique(x))
    }
  )
  ...
  test.alltime <- tapply(RR.coxph$times, RR.coxph$strata, function(x) {
    x
  })
  ...
  GS.alltime <- tapply(
    Melanoma$time, interaction(Melanoma$invasion, Melanoma$ici),
    function(x) {
      sort(unique(x))
    }
  )
  ...
  test.alltime <- tapply(RR.cph$times, RR.cph$strata, function(x) {
    x
  })
...
test_that("baseline hazard (no strata, time varying): compare to survival::basehaz", {
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fit.coxphTV, centered = FALSE)$cumhazard),
    basehaz(fit.coxphTV, centered = FALSE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fit.coxphTV, centered = TRUE)$cumhazard),
    basehaz(fit.coxphTV, centered = TRUE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fit.cphTV)$cumhazard),
...
test_that("baseline hazard (strata, time varying): compare to survival::basehaz", {
  ## strata defined by S
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS1.coxphTV, centered = FALSE)$cumhazard),
    basehaz(fitS1.coxphTV, centered = FALSE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS1.coxphTV, centered = TRUE)$cumhazard),
    basehaz(fitS1.coxphTV, centered = TRUE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS1.cphTV)$cumhazard),
    basehaz(fitS1.cphTV)$hazard, tolerance = 1e-8
  )

  ## strata defined by randomS
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS2.coxphTV, centered = FALSE)$cumhazard),
    basehaz(fitS2.coxphTV, centered = FALSE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS2.coxphTV, centered = TRUE)$cumhazard),
    basehaz(fitS2.coxphTV, centered = TRUE)$hazard, tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, suppressWarnings(predictCox(fitS2.cphTV)$cumhazard),
...
predRR1 <- predictCox(e.coxph,
  newdata = dt, times = 10,
  se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
)
predRR2 <- predictCox(e.coxph_sort,
  newdata = dt, times = 10,
  se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
)

predRR1.none <- confint(predRR1,
  survival.transform = "none", seed = 10,
  n.sim = n.sim
)
predRR1.loglog <- confint(predRR1,
  survival.transform = "loglog", seed = 10,
  n.sim = n.sim
)

## *** Test vs. cph
test_that("[predictCox] compare survival and survival.se coxph/cph (1 fixed time)", {
  res.cph <- predictCox(e.cph,
    newdata = dt, times = 10,
    se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
  )
  expect_equal(
    ignore_attr = TRUE, as.data.table(predRR1),
...
test_that("[confint.predictCox] compare to known values (eventtimes, log log transformation)", {
  ...
  ## butils::object2script(as.data.table(predRR1.loglog)[6:7,names(GS),with=FALSE], digit = 5)
  expect_equal(
    ignore_attr = TRUE, as.data.table(predRR1.loglog)[6:7, names(GS), with = FALSE],
...
test_that("[predictCox] - sorted vs. unsorted times (no strata)", {
  ...
  expect_equal(
    ignore_attr = TRUE, predRR1$survival[, index.sort],
    predRR2$survival
  )

  expect_equal(
    ignore_attr = TRUE, predRR1$survival.se[, index.sort],
    predRR2$survival.se
  )

  expect_equal(
    ignore_attr = TRUE, predRR1$time[index.sort],
    predRR2$time
  )
})
...
test_that("[predictCox] - fast iid average (no strata)", {
  ## simple average
  predRR.av <- predictCox(e.coxph,
    times = dt$time[1:5], average.iid = TRUE, newdata = dt,
    type = c("hazard", "cumhazard", "survival")
  )
  predRR.GS <- predictCox(e.coxph,
    times = dt$time[1:5], iid = TRUE, newdata = dt,
    type = c("hazard", "cumhazard", "survival")
  )

  expect_equal(
    ignore_attr = TRUE, apply(predRR.GS$hazard.iid, MARGIN = 1:2, mean),
    predRR.av$hazard.average.iid, tolerance = 1e-8
  )

  expect_equal(
    ignore_attr = TRUE, apply(predRR.GS$cumhazard.iid, MARGIN = 1:2, mean),
    predRR.av$cumhazard.average.iid, tolerance = 1e-8
  )

  expect_equal(
    ignore_attr = TRUE, apply(predRR.GS$survival.iid, MARGIN = 1:2, mean),
  ...
  fT <- TRUE
  attr(fT, "factor") <- list(
    matrix(5, nrow = NROW(dt), ncol = 5),
    matrix(1:NROW(dt), nrow = NROW(dt), ncol = 5)
  )

  predRR.av2 <- predictCox(e.coxph,
    times = sort(dt$time[1:5]), average.iid = fT, newdata = dt,
    type = c("hazard", "cumhazard", "survival")
  )
  calcGS <- function(iid) {
  ...
  expect_equal(
    ignore_attr = TRUE, predRR.av$hazard.average.iid[, order(dt$time[1:5])] * 5,
    predRR.av2$hazard.average.iid[[1]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predRR.av$cumhazard.average.iid[, order(dt$time[1:5])] * 5,
    predRR.av2$cumhazard.average.iid[[1]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predRR.av$survival.average.iid[, order(dt$time[1:5])] * 5,
    predRR.av2$survival.average.iid[[1]],
    tolerance = 1e-8
  )

  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$hazard.iid)[, order(dt$time[1:5])],
    predRR.av2$hazard.average.iid[[2]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$cumhazard.iid)[, order(dt$time[1:5])],
    predRR.av2$cumhazard.average.iid[[2]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$survival.iid)[, order(dt$time[1:5])],
...
predRR1 <- predictCox(eS.coxph,
  newdata = dtStrata, times = 4,
  se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
)
...
predRR1 <- predictCox(eS.coxph,
  newdata = dtStrata, times = vec.time,
  se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
)
...
test_that("[confint.predictCox] compare to known values (eventtimes, log log transformation, strata)", {
  ...
  expect_equal(
    ignore_attr = TRUE, as.data.table(predRR1.loglog)[135:136, names(GS), with = FALSE],
    GS, tolerance = 1e-4, scale = 1
  )
})
...
test_that("[predictCox] after the last event (strata)", {
  ...
  predRR1 <- predictCox(eS.coxph,
    newdata = dtStrata, times = max(lastevent[["V1"]]),
    se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
  )
  predRR1 <- confint(predRR1, n.sim = n.sim)
...
test_that("[predictCox] - iid average", {
  ...
  ## simple average
  predRR.av <- predictCox(eS.coxph,
    times = seqTime, average.iid = TRUE, newdata = dtStrata,
    type = c("hazard", "cumhazard", "survival")
  )
  predRR.GS <- predictCox(eS.coxph,
    times = seqTime, iid = TRUE, newdata = dtStrata,
    type = c("hazard", "cumhazard", "survival")
  )
  ...
  fT <- TRUE
  attr(fT, "factor") <- list(
    matrix(5, nrow = NROW(dtStrata), ncol = 6),
    matrix(1:NROW(dtStrata), nrow = NROW(dtStrata), ncol = 6)
  )

  predRR.av2 <- predictCox(eS.coxph,
    times = sort(seqTime), average.iid = fT, newdata = dtStrata,
    type = c("hazard", "cumhazard", "survival")
  )
  calcGS <- function(iid) {
    apply(iid, MARGIN = 1:2, function(iCol) {
  ...
  expect_equal(
    ignore_attr = TRUE, predRR.av$hazard.average.iid[, order(seqTime)] * 5,
    predRR.av2$hazard.average.iid[[1]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predRR.av$cumhazard.average.iid[, order(seqTime)] * 5,
    predRR.av2$cumhazard.average.iid[[1]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, predRR.av$survival.average.iid[, order(seqTime)] * 5,
    predRR.av2$survival.average.iid[[1]],
    tolerance = 1e-8
  )

  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$hazard.iid)[, order(seqTime)],
    predRR.av2$hazard.average.iid[[2]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$cumhazard.iid)[, order(seqTime)],
    predRR.av2$cumhazard.average.iid[[2]],
    tolerance = 1e-8
  )
  expect_equal(
    ignore_attr = TRUE, calcGS(predRR.GS$survival.iid)[, order(seqTime)],
    predRR.av2$survival.average.iid[[2]],
    tolerance = 1e-8
  )
})
...
fit.pred <- predictCox(fit,
  newdata = dt[1:3], times = c(3, 8), type = "survival",
  se = TRUE, iid = TRUE, band = TRUE, confint = FALSE
)
...
test_that("[predictCox] consistency  iid se", {
  expect_equal(
    ignore_attr = TRUE, fit.pred$survival.se[1, ],
...
test_that("[confint.predictCox] manual computation on ci", {
  ## orignial scale
  expect_equal(
    ignore_attr = TRUE, confint.pred1$survival.lower,
    fit.pred$survival + qnorm(0.025) * fit.pred$survival.se
  )
  expect_equal(
    ignore_attr = TRUE, as.double(confint.pred1$survival.upper),
  ...
  expect_equal(
    ignore_attr = TRUE, confint.pred2$survival.lower,
    exp(-exp(log(-log(fit.pred$survival)) + qnorm(0.975) * newse))
  )
  expect_equal(
    ignore_attr = TRUE, confint.pred2$survival.upper,
...
test_that("[predictCox] diag no strata", {
  ...
  test <- predictCox(e.coxph,
    newdata = dt, times = dt$time,
    se = TRUE, iid = TRUE, average.iid = TRUE, diag = TRUE
  )
  test2 <- predictCox(e.coxph,
    newdata = dt, times = dt$time,
  ...
  ## iid
  GS.iid.diag <- do.call(cbind, lapply(
    1:NROW(dt),
    function(iN) {
      GS$survival.iid[, iN, iN]
    }
  ))
  ...
  average.iid <- TRUE
  attr(average.iid, "factor") <- list(
    matrix(1:length(dt$time), nrow = NROW(dt), ncol = length(dt$time), byrow = TRUE),
    matrix(1:NROW(dt), nrow = NROW(dt), ncol = length(dt$time))
  )
  test3 <- predictCox(e.coxph,
    newdata = dt, times = dt$time,
  ...
  expect_equal(
    ignore_attr = TRUE, apply(GS$survival.iid, 1:2, function(x) {
      sum(x * (1:length(dt$time))) / length(x)
    }),
  ...
  test4 <- predictCox(e.coxph,
    newdata = dt, times = dt$time,
    se = FALSE, iid = FALSE, average.iid = average.iid, diag = FALSE
  )
  ## iObs <- 1
  expect_equal(
    ignore_attr = TRUE, do.call(rbind, lapply(1:NROW(dt), function(iObs) {
      rowMeans(GS$survival.iid[iObs, , ] * t(attr(average.iid, "factor")[[1]]))
    })),
  ...
  average.iid <- TRUE
  attr(average.iid, "factor") <- list(
    matrix(5, nrow = NROW(dt), ncol = 1, byrow = TRUE),
    matrix(1:NROW(dt), nrow = NROW(dt), ncol = 1)
  )
  test5 <- predictCox(e.coxph,
    newdata = dt, times = dt$time,
  ...
  expect_equal(
    ignore_attr = TRUE, rowMeans(rowMultiply_cpp(GS.iid.diag, 1:length(dt$time))),
...
test_that("[predictCox] diag strata", {
  ...
  test <- predictCox(eS.coxph,
    newdata = dt, times = dt$time,
    se = FALSE, iid = TRUE, average.iid = TRUE, diag = TRUE
  )
  test2 <- predictCox(eS.coxph,
    newdata = dt, times = dt$time,
    se = FALSE, iid = FALSE, average.iid = TRUE, diag = TRUE
  )
...
test_that("[predictCox] Quantile for the confidence band of the cumhazard", {
  ...
  pred.band2 <- riskRegression:::quantileProcess_cpp(
    nSample = dim(predRR$cumhazard.iid)[1],
  ...
  set.seed(10)
  pred.band2.no0 <- riskRegression:::quantileProcess_cpp(
    nSample = dim(predRR$cumhazard.iid)[1],
    nContrast = dim(predRR$cumhazard.iid)[3],
    nSim = n.sim,
...
eS.pred <- predictCox(eS.coxph,
  newdata = dtStrata, times = 1:4,
...
test_that("[predictCox] store.iid = minimal vs. full - no strata", {
  res1 <- predictCox(m.coxph,
    times = seqTime, newdata = newdata,
    type = c("cumhazard", "survival"),
    store.iid = "minimal", se = TRUE, iid = TRUE, average.iid = TRUE
  )
  res2 <- predictCox(m.coxph,
    times = seqTime, newdata = newdata,
    type = c("cumhazard", "survival"),
...
test_that("Deal with negative/NA time points", {
  expect_equal(
    ignore_attr = TRUE, unname(predictCox(fit.coxph, times = -1, newdata = Melanoma)$survival),
...
test_that("Standard error after atanh transformation", {
  ...
  e.trans <- transformCIBP(
    estimate = 0.5, se = cbind(1), null = 0,
...
test_that("Cox - iid/se should not depend on other arguments", {
  ...
  seqTau <- abs(rnorm(10))
  out1 <- predictCox(fit,
    newdata = d[1:5], times = seqTau,
    se = TRUE, iid = TRUE, average.iid = TRUE
  )
  out2 <- predictCox(fit,
    newdata = d[1:5], times = sort(seqTau),
    se = TRUE, iid = TRUE, average.iid = TRUE
  )
