...
test_that("start stop Cox: basehaz vs timereg", {
  ...
  expect_equal(
    ignore_attr = TRUE, 1 - predictRisk(cox, newdata = veteran[c(47, 8, 17, 10), ], times = test.times),
  ...
  expect_gt(
    unlist(riskRegression:::predictRisk.coxphTD(coxTD, newdata = vet2[c(17), ], landmark = 60, times = 48)),
