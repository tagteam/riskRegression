...
test_that("survival - several strata variables", {
  ...
  expect_equal(
    ignore_attr = TRUE, predictCox(mS.phreg, center = FALSE)[c("cumhazard", "survival")],
...
test_that("competing risk - no strata", {
  m.phreg <- CSC(Hist(time, event) ~ X1 + X2,
    data = d,
    fitter = "phreg"
  )
  m.coxph <- CSC(Hist(time, event) ~ X1 + X2,
    data = d,
    fitter = "coxph"
  )
...
test_that("competing risk - one strata variable", {
  mS.phreg <- CSC(Hist(time, event) ~ X1 + X2 + strata(group) + cluster(id),
    data = d,
    fitter = "phreg"
  )
  mS.coxph <- CSC(Hist(time, event) ~ X1 + X2 + strata(group),
    data = d,
    fitter = "coxph"
  )
...
test_that("competing risk - several strata variables", {
  mS.phreg <- CSC(Hist(time, event) ~ X1 + X2 + strata(group, X3) + cluster(id),
    data = d,
    fitter = "phreg"
  )
  mS.coxph <- CSC(Hist(time, event) ~ X1 + X2 + strata(group) + strata(X3),
    data = d,
    fitter = "coxph"
  )
  expect_equal(ignore_attr = TRUE, class(mS.phreg$models[[1]]), "phreg")
