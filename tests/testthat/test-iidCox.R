...
test_that("[iidCox] iid hazard = 0 at non event times and NA after last observation", {
  ...
  expect_equal(
    ignore_attr = TRUE, iid.test$IFhazard[[1]][, as.character(iid.coxph$time[[1]])],
  ...
  vec.time <- intersect(
    iid.test$time[[1]][iid.test$time[[1]] > timeLastEvent],
...
e.coxph <- coxph(Surv(time, event) ~ strata(X1),
  data = dtS,
  x = TRUE, y = TRUE
)
test_that("[iidCox] empty strata", {
  ...
  GS <- cbind(
    "0.2" = c(0, 0, 0, -0.00173611, 0.03993056, 0, 0, -0.00173611, 0, 0, 0, 0, -0.00173611, -0.00173611, -0.00173611, 0, -0.00173611, 0, 0, 0, 0, -0.00173611, -0.00173611, -0.00173611, 0, 0, 0, -0.00173611, -0.00173611, -0.00173611, 0, -0.00173611, 0, -0.00173611, -0.00173611, -0.00173611, -0.00173611, 0, 0, -0.00173611, 0, 0, -0.00173611, 0, 0, -0.00173611, -0.00173611, -0.00173611, 0, -0.00173611),
...
eApprox.timereg <- cox.aalen(Surv(time, event) ~ prop(X1) + prop(X6),
  data = dt, resample.iid = TRUE, max.timepoint.sim = NULL,
  rate.sim = 0, clusters = seq_len(NROW(dt))
)
...
test_that("[iidCox] lambda - no strata, interactions, categorical", {
  expect_equal(
    ignore_attr = TRUE, as.double(IF.coxph$IFcumhazard[[1]]),
...
e.coxph_Efron <- coxph(Surv(timeTies, event) ~ X1 + X6,
  data = dtTies, y = TRUE, x = TRUE,
  ties = "efron"
)
e.coxph_Breslow <- coxph(Surv(timeTies, event) ~ X1 + X6,
  data = dtTies, y = TRUE, x = TRUE,
  ties = "breslow"
)
...
## *** Model
e.timereg <- cox.aalen(Surv(time, event) ~ strata(strata) - 1 + prop(X1) + prop(X6),
  data = dtStrata,
...
test_that("[iidCox] lambda - strata", {
  ...
  for (iStrata in 1:n.strata) { ## iStrata <- 1
    IF.GS <- do.call(
      rbind,
      lapply(e.timereg$B.iid, function(x) {
        x[, iStrata]
      })
    )
    colnames(IF.GS) <- e.timereg$time.sim.resolution

    checkTimes <- intersect(
      e.timereg$time.sim.resolution,
...
test_that("[iidCox] Compare to timereg on Melanoma dta", {
  ...
  expect_equal(
    ignore_attr = TRUE, as.double(RR.iid$IFcumhazard[[1]]), ## as.double(RR.iid$IFcumhazard[[1]])[c(1:4,206:209,411)]
    as.double(timereg.iidLambda[, -1])
  ) ## as.double(timereg.iidLambda[,-1])[c(1:4,206:209,411)]
})
#----------------------------------------------------------------------
