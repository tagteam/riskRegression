...
test_that("predict.CSC(2b) - compare to mstate",{
    ...
    expect_equal(as.double(pred.RR1a$absRisk.se),pred.probtrans[,"se2"], tol = 6e-3)
    expect_equal(as.double(pred.RR1b$absRisk.se),pred.probtrans[,"se2"], tol = 6e-3)
    expect_equal(as.double(pred.RR1c$absRisk.se),pred.probtrans[,"se3"], tol = 6e-3)
    expect_equal(as.double(pred.RR1d$absRisk.se),pred.probtrans[,"se3"], tol = 6e-3)
    expect_equal(as.double(pred.RR2a$absRisk.se),pred.probtrans[,"se2"], tol = 5e-3)    
    expect_equal(as.double(pred.RR2b$absRisk.se),pred.probtrans[,"se2"], tol = 5e-2)        
})
...
test_that("predictCSC with strata",{
    ...
    EventTest.Sse <- rbind(c(0, 0, 0.010446473432358, 0.010446473432358, 0.0436200361892591, 0.0510348037860702, 0.0666492502747174, 0.0643776679656854, NA, NA),
                           c(0, 0, 0.0082531430250501, 0.0082531430250501, 0.0370331720961396, 0.0450714870936402, 0.0658982142655854, 0.0662972715561105, NA, NA),
                           c(0, 0, 0, 0, 0.0135491573453514, 0.0211438835120505, 0.0454399510332472, 0.0745077199976662, 0.078929172496032, NA),
                           c(0, 0, 0.00509273689287847, 0.00509273689287847, 0.0261433701357819, 0.0340266474213875, 0.0587857768831935, 0.063312347114687, NA, NA),
                           c(0, 0, 0.0209835473468991, 0.0209835473468991, 0.0574099003150858, 0.0744242731646183, 0.0767880946287887, 0.0735261309494099, NA, NA),
                           c(0, 0, 0.0233576982126292, 0.0233576982126292, 0.0619743510257844, 0.0784942043407909, 0.0781534584441169, 0.0696094851982638, NA, NA),
                           c(0, 0, 0, 0, 0.016220503254504, 0.0250190323209576, 0.0521133112858053, 0.0785527791175938, 0.0803916212033726, NA),
                           c(0, 0, 0, 0, 0.0221968165692495, 0.0295952724637807, 0.0451607193364939, 0.0656308363935428, NA, NA),
                           c(0, 0, 0.00154159465310257, 0.00154159465310257, 0.0104860575513312, 0.0151695919092928, 0.0320056430207543, 0.0376376409194765, NA, NA),
                           c(0, 0, 0.00758847653478711, 0.00758847653478711, 0.0348954545623498, 0.0430218225051946, 0.0651210809983647, 0.0664059478434416, NA, NA))
    ...
    # exportRes(Event0.S$absRisk.se)
    EventTest.Sse <- rbind(c(0, 0, 0, 0.0149156347784489, 0.0390659454703574, 0.0419061497350761, 0.0535338408672505, 0.0571145306111307, NA, NA),
                           c(0, 0, 0, 0.0147833490043145, 0.0402092363670696, 0.0434201836780092, 0.0581444321701597, 0.0619718998015213, NA, NA),
                           c(0, 0, 0, 0.0259918754688021, 0.0259918754688021, 0.0412031937569182, 0.0739975752859448, 0.0894187465200922, 0.0894187465200922, NA),
                           c(0, 0, 0, 0.0145353530186353, 0.0427750004778479, 0.0467633458119166, 0.0690344216809415, 0.0736722835020863, NA, NA),
                           c(0, 0, 0, 0, 0, 0, 0.0699616881145798, 0.0699616881145798, NA, NA),
                           c(0, 0, 0, 0, 0, 0, 0.0669324733763614, 0.0669324733763614, NA, NA),
                           c(0, 0, 0, 0.0263129877100383, 0.0263129877100383, 0.0415063774673616, 0.0732563986625892, 0.0869041845416013, 0.0869041845416013, NA),
                           c(0, 0, 0, 0, 0.0336515620082026, 0.0451088790250896, 0.0686836749430134, 0.0734589225097384, NA, NA),
                           c(0, 0, 0, 0.0140823505761779, 0.0503023680123357, 0.0563051302936449, 0.0992186839792852, 0.10907901002636, NA, NA),
                           c(0, 0, 0, 0.0147378473695602, 0.0406333970102112, 0.0439778784882177, 0.0599185471891525, 0.0638436470582947, NA, NA))
