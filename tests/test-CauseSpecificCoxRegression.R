...
test_that("CSC vs prodlim",{
    ...
    pB <- predictRisk(B,times=sort(unique(Melanoma$time)),newdata=nd[1,],cause=1)
    pA <- predictRisk(A,times=sort(unique(Melanoma$time)),newdata=nd[1,],cause=1)
    ...
    c <- CSC(Hist(time,status)~strat(sex),data=Melanoma,survtype="survival",fitter="cph")
