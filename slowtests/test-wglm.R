...
test_that("wglm - no censoring",{
    ...
    GS <- suppressWarnings(logitIPCW(formula = Event(time,event)~X1 + X8, time = tau[5], data = dFull))
...
test_that("wglm - censoring",{
    ...
    GS <- suppressWarnings(logitIPCW(formula = Event(time,event)~X1 + X8, time = tau[5], data = dSurv, cens.code = 0, cause = 1))
    expect_equal(ignore_attr=TRUE,coef(test, time = tau[5]), summary(GS)$coef[,"Estimate"], tolerance = 1e-5)
    expect_equal(ignore_attr=TRUE,summary(test, print = FALSE)[[5]][,"Std. Error"], summary(GS)$coef[,"Std.Err"], tolerance = 1e-5)

    ## check weights
    e.KM <- as.data.table(predictCox(coxph(Surv(time,event==0) ~ 1, data = dSurv), type = c("hazard","survival"), product.limit = TRUE))
    ...
    test <- wglm(Surv(time,event) ~ X1 + X8, formula.censor = ~ X1,
                 times = tau, data = dSurv, product.limit = TRUE)
    
    GS <- suppressWarnings(logitIPCW(formula = Event(time,event) ~ X1 + X8, cens.model = ~strata(X1),
                                     time = tau[5], data = dSurv, cens.code = 0, cause = 1))

    expect_equal(ignore_attr=TRUE, coef(test, time = tau[5]), summary(GS)$coef[,"Estimate"], tolerance = 1e-5)

    #### censoring model with continuous covariate ####
    test <- wglm(Surv(time, event) ~ X1 + X8, formula.censor = ~ X1 + X8,
                 times = tau, data = dSurv, product.limit = FALSE, fitter = "coxph")
    GS <- suppressWarnings(logitIPCW(formula = Event(time,event) ~ X1 + X8, cens.model = ~X1+X8,
                    time = tau[5], data = dSurv, cens.code = 0, cause = 1))
...
test_that("wglm - competing risks",{
    ...
    #### no covariate in censoring model ####
    test <- wglm(Surv(time,event) ~ X1 + X8,
...
##         iModelRR <- wglm(Surv(time,event) ~ X1 + X8, formula.censor =  ~ X1+X8,
