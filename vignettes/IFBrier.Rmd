...
\newcommand{\LL}{\{\I{\T_{m+1}\leq \tau}-\R(X_{m+1})\}^2}
...
\(m\). The expectation is taken with respect to the data of subject \(m+1\):
\begin{align*}
\psi_{m}(P)&=\int\left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2
\prod_{i=1}^m P(d o_i)\right) P(d
o_{m+1}) \end{align*}

for some sample size $m < n$. Let 
\begin{equation*}
  \omega_{m} (Y_{m+1},X_{m+1})=\mathbb{E}_{D_m} [(Y_{m+1}-R(D_m)(X_{m+1}))^2 | Y_{m+1}, X_{m+1}] 
\end{equation*}
because then $\psi_{m}(P) = \mathbb{E}_{O_{m+1}}[\omega_{m} (Y_{m+1},X_{m+1})]$. For estimating $\omega_{\tau,m}$, we propose to use leave one-out bootstrap estimation:
\begin{equation*}
...
\begin{align} 
  ...
  &= \int \frac{\partial}{\partial\epsilon} \left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2 \prod_{i=1}^m \PEK (d o_i)\right) \PEK(d o_{m+1}) \Big\vert_{\epsilon=0} \\
  &= \int \frac{\partial}{\partial\epsilon} \left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2 \prod_{i=1}^m \PEK (d o_i)\right) \Big\vert_{\epsilon=0}  P(d o_{m+1}) \\
  &+  \int \left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2 \prod_{i=1}^m P (d o_i)\right) \frac{\partial}{\partial\epsilon} \PEK(d o_{m+1}) \Big\vert_{\epsilon=0}  \\
  &= \int \sum_{j=1}^m \left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2  \delta_{o_k}(o_j) \prod_{i \neq j} (d o_i)\right)  P(d o_{m+1})  - m \psi_{m}(P) \\ %notation???
  &+ \omega_{m} (Y_{k},X_{k})-\psi_{m}(P) \\
  &= \omega_{m} (Y_{k},X_{k})-(m+1) \psi_{m}(P) \\
  &+\int \sum_{j=1}^m \left(\int\cdots\int\left\{y_{m+1}-R(\Dm)(x_{m+1})\right\}^2  \delta_{o_k}(o_j) \prod_{i \neq j} (d o_i)\right)  P(d o_{m+1}) 
\end{align}
wherein we used the product rule of differentiation. For estimating the influence function, we suggest the estimator:
\begin{align*}
  \widehat{\IC}_{\psi}(m;O_k)&=\mbin (Y_k,X_k)-(m+1)\, \PSI +\frac{1}{n} \sum_{i=1}^n\frac{\sum_b \{Y_i-\Rbi (X_i)\}^2 \Ibi N_k^b}{\sum_b\Ibi} % is the last part estimated correctly?
\end{align*}
...
\begin{equation*}
  \mu_{\tau,m}=\E_{O_{m+1}}\big[\E_{D_{m}}\big[\LL\big|\T_{m+1},X_{m+1}\big]\big]
\end{equation*} 
...
\begin{align*}
  \psi_{\tau,m}(\P)&=\int\left(\int\cdots\int\left\{\I{u_{m+1}\leq\tau}-\RR(\Dm)(x_{m+1})\right\}^2  \prod_{i=1}^m\P(\D o_i)\right)\\
   &\qquad\qquad\times\W(o_{m+1},\kappa_{\tau,x_{m+1}}(\P))\P(\D o_{m+1})\\&=\mu_{\tau,m}.
\end{align*}
with 
\begin{align*}
...
\begin{align*} 
  \IC_{\psi}(\tau,m;o_k)&=\M(t_k,x_k)\W(o_k,\kappa_{\tau,x_{m+1}}(\P))-(m+1)\, \mu_{\tau,m}\\
  &+\int\Big[\sum_{j=1}^m \int\cdots\int\{\I{t_{m+1}\leq\tau}-\RR(\{o_i\}_{i=1}^n)(x_{m+1})\}^2  \delta_{o_k}(o_j)\prod_{i\neq j}\P(\D o_i)\Big]\W(o_{m+1},\kappa_{\tau,x_{m+1}}(\P))\P(\D o_{m+1})
  \\
  &+\int\M(t_{m+1},x_{m+1})\left[\frac{\I{t_{m+1}\leq \tau}\delta_{m+1}}{G(t_{m+1}-\vert x_{m+1})} f_k(t_{m+1},x_{m+1})
  +\frac{\I{t_{m+1}>\tau}}{G(\tau\vert x_{m+1})}f_k(\tau,x_{m+1}) \right]\P(\D o_{m+1}) \\
\end{align*}
This can then be estimated in much the same way as before
\begin{align*}
  \widehat{\IC}_{\psi}(\tau,m;O_k)&=\m(T_k,X_k)\W(O_k,\G)-(m+1)\, \MU \\
  &+\frac{1}{n} \sum_{i=1}^n\frac{\sum_b \{\I{T_i\leq \tau}-\Rb(X_i)\}^2 \Ibi N_k^b}{\sum_b\Ibi}\W(O_i,\G)\\
