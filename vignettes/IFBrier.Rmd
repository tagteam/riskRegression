...
bootstrap dataset \(\Db=\{X_{b,1}^*,\ldots,X_{b,m}^*\}\) of size
...
following functional, i.e., the expected Brier score of the model
\(R\) on average across all possible training datasets of size
\(m\). The expectation is taken with respect \begin{align*}
\psi_{m}(P)&=\int\left(\int\cdots\int\left\{y_{m+1}-R(\{x_i\}_{i=1}^m)(z_{m+1})\right\}^2
I(x_{m+1} \notin \{x_i\}_{i=1}^m)\prod_{i=1}^m P(d x_i)\right) P(d
x_{m+1}) \end{align*}
