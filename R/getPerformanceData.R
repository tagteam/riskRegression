...
getPerformanceData <- function(testdata,
  ...
  args <- switch(response.type,
    "binary" = {
      list(newdata = X)
    },
    "survival" = {
      list(newdata = X, times = times)
    },
    "competing.risks" = {
      list(newdata = X, times = times, cause = cause)
    },
  ...
  pred <- data.table::rbindlist(lapply(levs, function(f) {
    ...
    if (f[1] > 0 && (length(extra.args <- unlist(lapply(object.classes[[f]], function(cc) {
      predictRisk.args[[cc]]
    }))) > 0)) {
