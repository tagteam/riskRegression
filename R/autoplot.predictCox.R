...
#'
#' ## predictions
#' pred.cox.strata <- predictCox(m.cox.strata,
#'   newdata = d[1:3, , drop = FALSE],
...
#' )
#' grid <- data.frame(
#'   X1 = factor(0, 0:1), X2 = factor(0, 0:1),
#'   X6 = seq(min(d$X6), max(d$X6), length.out = 100)
#' )
#' pred.spline <- predictCox(m.cox.spline,
#'   newdata = grid, keep.newdata = TRUE,
...
#'
#' grid2 <- data.frame(
#'   X1 = factor(1, 0:1), X2 = factor(0, 0:1),
#'   X6 = seq(min(d$X6), max(d$X6), length.out = 100)
#' )
#' pred.spline <- predictCox(m.cox.spline,
#'   newdata = rbind(grid, grid2), keep.newdata = TRUE,
#'   se = TRUE, band = TRUE, centered = TRUE, type = "lp"
#' )
...
autoplot.predictCox <- function(object,
  ...
  if (is.null(ylab)) {
    if (first.derivative) {
      ylab <- switch(type,
        "cumhazard" = if (object$baseline && nVar.lp > 0) {
          "instantaneous baseline hazard"
        } else {
          "instantaneous hazard"
        },
        "survival" = if (object$baseline && nVar.lp > 0) {
          "derivative of the baseline survival"
        } else {
  ...
  if (group.by[[1]] == "covariates" && ("newdata" %in% names(object)) == FALSE) {
    stop(
      "argument \'group.by\' cannot be \"covariates\" when newdata is missing in the object \n",
  ...
  if (group.by[[1]] %in% c("strata") && ("strata" %in% names(object) == FALSE)) {
    stop(
      "argument \'group.by\' cannot be \"strata\" when strata is missing in the object \n",
  ...
  if (group.by[[1]] %in% c(object$var.lp, object$var.strata) && ("newdata" %in% names(object) == FALSE)) {
    stop(
      "argument \'group.by\' cannot be \"", group.by[[1]], "\" when newdata is missing in the object \n",
      "set argment \'keep.newdata\' to TRUE when calling the predictCox function \n"
    )
  }
  if (!is.null(object$newdata$strata) && !is.null(object$var.strata)) {
    if (any(object$var.strata %in% colnames(object$newdata))) {
      stop(
        "cannot handle covariate \"", paste(object$var.strata[object$var.strata %in% colnames(object$newdata)], collapse = "\" \""), "\" as this name is used internally.\n",
  ...
  if (ci[[1]] == TRUE && (object$se[[1]] == FALSE || is.null(object$conf.level))) {
    stop(
      "argument \'ci\' cannot be TRUE when no standard error have been computed \n",
  ...
  if (band[[1]] && (object$band[[1]] == FALSE || is.null(object$conf.level))) {
    stop(
      "argument \'band\' cannot be TRUE when the quantiles for the confidence bands have not been computed \n",
      "set arguments \'band\' and \'confint\' to TRUE when calling the predictCox function \n"
    )
  }
  if (object$nTimes != 0 && any(rank(object$times) != seq_len(length(object$times)))) {
  ...
  if (type == "lp") {
    ...
    group.by <- if (length(group.by[-1]) == 0) {
      "row"
    } else {
      group.by[-1]
    }
    ...
    if (!is.matrix(object[[type]])) {
      ...
      if (is.null(object[["strata"]])) {
        ...
        if (object$nTimes == 0) {
          if (0 %in% object$times == FALSE) {
            ...
            if (!is.null(object$newdata)) {
              object$newdata <- rbind(
                data.table(start = 0, stop = 0, status = NA, strata = 1, strata.num = 0, eXb = NA, statusM1 = NA, XXXindexXXX = NA),
                object$newdata
              )
            }
          }
          if (object$lastEventTime %in% object$times == FALSE) {
            ...
            if (!is.null(object$newdata)) {
              object$newdata <- rbind(
                data.table(start = 0, stop = object$lastEventTime, status = 0, strata = 1, strata.num = 0, eXb = NA, statusM1 = NA, XXXindexXXX = NA),
                object$newdata
              )
            }
          }
        ...
        for (iStrata in 1:n.strata) { ## iStrata <- 1
          ...
          type.tempo[iStrata, ] <- stats::approx(
            x = object[["times"]][index.strata],
      ...
      if (!is.null(newdata) && reduce.data[[1]] == TRUE) {
        test <- unlist(newdata[, lapply(.SD, function(col) {
          length(unique(col)) == 1
        })])
        if (any(test)) {
    ...
    dataL <- predict2melt(
      outcome = object[[type]], ci = ci, band = band,
      outcome.lower = if (ci) {
      ...
      outcome.lowerBand = if (band) {
        object[[paste0(type, ".lowerBand")]]
      } else {
        NULL
      },
      outcome.upperBand = if (band) {
  ...
  gg.res <- predict2plot(
    ...
    alpha = alpha,
    smooth = smooth,
    xlab = if (is.null(object$infoVar)) {
      "time"
    } else {
      object$infoVar$time
    },
...
predict2melt <- function(outcome, name.outcome,
  ...
  if (!is.null(status)) {
    ...
    M.status <- matrix(as.numeric(NA),
      nrow = NROW(outcome), ncol = NCOL(outcome),
      dimnames = list(NULL, paste0("status_", time.names))
    )
    status[, c("index") := match(.SD$stop, times)]

    for (iS in seq_len(length(Ustrata))) { ## iS <- 1
  ...
  if (!is.null(newdata)) { ## predicted survival/risk/average treatment effect
    ...
    keep.col <- c(keep.col, if (!is.null(cov.names)) {
      "covariates"
    }, cov.names)
  }
...
predict2plot <- function(dataL, name.outcome,
  ...
  if (smooth >= 1) {
    ...
    if (first.derivative) {
      ...
      warper <- function(data) { ## data <- dataL[row==1]
        ...
        return(numDeriv::grad(function(x) {
          predict(iModel, newdata = data.frame(time = x), type = "response")
        }, x = data$time))
      }
  ...
  if (band) { ## confidence band
    if (smooth > 0) {
      if (!is.na(alpha)) {
        gg.base <- gg.base + ggplot2::geom_ribbon(
        ...
        gg.base <- gg.base + ggplot2::geom_line(
          eval(parse(text = paste0(
      ...
      if (!is.na(alpha)) {
        gg.base <- gg.base + ggplot2::geom_rect(ggplot2::aes_string(
          xmin = "time", xmax = "timeRight", ymin = "lowerBand", ymax = "upperBand",
          fill = "labelBand"
        ), linetype = 0, alpha = alpha)
  ...
  if (smooth > 0) {
    gg.base <- gg.base + ggplot2::geom_line(
      mapping = ggplot2::aes_string(x = "time", y = paste0(name.outcome, ".smooth"), group = group.by, color = group.by),
      size = size.estimate
    )
  } else {
    gg.base <- gg.base + ggplot2::geom_segment(
      mapping = ggplot2::aes_string(x = "timeRight", y = name.outcome, xend = "time", yend = name.outcome, color = group.by),
      size = size.estimate
    )
    if ("status" %in% names(dataL)) {
      dataL$status <- as.character(dataL$status)
      gg.base <- gg.base + ggplot2::geom_point(
        data = na.omit(dataL),
  ...
  if (group.by == "row") {
    gg.base <- gg.base + ggplot2::labs(color = "observation") + ggplot2::theme(
