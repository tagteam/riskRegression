...
plotCalibration <- function(x,
  ...
  if (is.character(legend[[1]]) || legend[[1]] == TRUE || auc.in.legend == TRUE || brier.in.legend == TRUE) {
    ...
    if (NCOL(legend.data) > 1) {
      addtable2plot.DefaultArgs <- list(
        yjust = 1.18,
        cex = cex,
  ...
  if (bars) {
    ...
    control <- prodlim::SmartControl(
      ...
      ignore.case = TRUE,
      defaults = list(
        "plot" = plot.DefaultArgs, "pseudo" = pseudo.DefaultArgs, "rug" = rug.DefaultArgs,
        "addtable2plot" = addtable2plot.DefaultArgs,
  ...
  if (method == "nne") {
    out <- c(out, list(bandwidth = sapply(
      plotFrames,
      ...
      lapply(1:NF, function(f) {
        ...
        if (showPseudo) {
          if (!is.null(control$pseudo$density) & control$pseudo$density > 0) {
            control$pseudo$col <- prodlim::dimColor(
              control$pseudo$col[f],
