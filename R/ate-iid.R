...
iidATE <- function(estimator,
  ...
  if (attr(estimator, "integral")) {
    ls.F1tau_F1t <- lapply(1:n.times, function(iT) {
      -colCenter_cpp(F1.jump, center = F1.tau[, iT])
    })
    ...
    ls.F1tau_F1t_SG <- lapply(1:n.times, function(iT) {
      ls.F1tau_F1t[[iT]] * iSG
    })
    ls.F1tau_F1t_dM_SGG <- lapply(1:n.times, function(iT) {
      ls.F1tau_F1t[[iT]] * dM_SGG
    })
    ls.F1tau_F1t_dM_SSG <- lapply(1:n.times, function(iT) {
      ls.F1tau_F1t[[iT]] * dM_SSG
    })
  }
  ...
  if (test.IPCW) {
    ...
    for (iTime in 1:n.times) { ## iTime <- 1
      attr(factor, "factor") <- lapply(1:n.contrasts, function(iC) {
        cbind(-iW.IPTW[, iC] * iW.IPCW2[, iTime] * Y.tau[, iTime])
      })

      term.censoring <- attr(predictRisk(object.censor,
        newdata = mydata, times = c(0, time.jumpC)[index.obsSINDEXjumpC[, iTime] + 1],
        diag = TRUE, product.limit = product.limit, average.iid = factor
      ), "average.iid")
  ...
  if (attr(estimator, "integral")) {
    ...
    term.intF1_tau <- attr(predictRisk(object.event,
      newdata = mydata, times = times, cause = cause,
      average.iid = factor, product.limit = product.limit
    ), "average.iid")
    ...
    integrand.F1t <- attr(predictRisk(object.event,
      newdata = mydata, times = time.jumpC, cause = cause,
      average.iid = factor, product.limit = product.limit
    ), "average.iid")
    ...
    integrand.St <- attr(predictRisk(object.event,
      type = "survival", newdata = mydata, times = time.jumpC - tol, cause = cause,
      average.iid = factor, product.limit = product.limit
    ), "average.iid")
    ...
    integrand.G1 <- predictCox(object.censor,
      newdata = mydata, times = time.jumpC - tol,
      average.iid = factor
    )$survival.average.iid
    ...
    integrand.G2 <- predictCox(object.censor,
      newdata = mydata, times = time.jumpC, type = "hazard",
      average.iid = factor
    )$hazard.average.iid
...
iidATE2 <- function(estimator,
  ...
  if (attr(estimator, "integral")) {
    ...
    ## **** survival model
    for (iTau in 1:n.times) { ## iTau <- 1
      integral.Surv <- calcItermIn(
        factor = -ls.F1tau_F1t[[iTau]] * dM_SG / S.jump,
    ...
    for (iTau in 1:n.times) { ## iTau <- 1
      ## integral censoring denominator
      integral.G <- calcItermIn(
        factor = -ls.F1tau_F1t[[iTau]] * dM_SG / G.jump,
      ...
      ## integral censoring martingale
      integral.dLambda <- calcItermIn(
        factor = -ls.F1tau_F1t[[iTau]] / SG,
