...
#' ## (argument band = TRUE)
#' ateFit1b <- ate(fit,
#'   data = dtS, treatment = "X1", times = 5:8,
#'   band = TRUE
#' )
#' summary(ateFit1b)
#'
...
#' ateFit1b <- ate(fit,
#'   data = dtB, treatment = "X1",
#'   times = 5
#' ) ## just for having a nice output not used in computations
...
#' ateFit2a <- ate(fitCR,
#'   data = dt, treatment = "X1", times = c(1, 5, 10),
...
#' )
#' resTD1 <- ate(cox1TD,
#'   formula = Hist(entry = start, time = time, event = status) ~ 1,
...
#' set.seed(16)
#' resTD <- ate(cscTD,
#'   formula = Hist(entry = start, time = time, event = event) ~ 1,
#'   data = d, treatment = "X3", contrasts = NULL,
#'   times = .5, verbose = 1,
...
ate <- function(event,
  ...
  out <- list(
    ...
    n = n.obs,
    variables = c(
      time = eventVar.time, event = eventVar.status,
    ...
    inference = data.frame(
      "se" = se, "iid" = iid, "band" = band, "bootstrap" = B > 0, "ci" = FALSE, "p.value" = FALSE,
      "conf.level" = NA, "alternative" = NA,
      "bootci.method" = NA, "n.bootstrap" = if (B > 0) {
        B
      } else {
        NA
      },
  ...
  if (!is.na(eventVar.time)) {
    ...
    if (attr(estimator, "TD")) {
      attr(out$eval.times, "n.at.risk") <- dcast(
        cbind(
          times = paste0(times, "+", landmark),
          data[data[[var.group]] %in% contrasts,
            list(pc = sapply(times + landmark, function(t) {
              sum(.SD[[eventVar.time]] >= t)
            })),
            by = var.group
          ]
        ),
      ...
      attr(out$eval.times, "n.at.risk") <- dcast(
        cbind(
          times = times,
          data[data[[var.group]] %in% contrasts,
            list(pc = sapply(times, function(t) {
              sum(.SD[[eventVar.time]] >= t)
            })),
            by = var.group
          ]
  ...
  if (se || band || iid) {
    ...
    if (B > 0) {
      ...
      ## display start
      if (verbose > 1 / 2) {
        cat(" - Non-parametric bootstrap using ", B, " samples and ", mc.cores, " core", if (mc.cores > 1) {
          "s"
        }, "\n", sep = "")
      ...
      name.estimate <- c(
        paste("mean", out$meanRisk$estimator, out$meanRisk$time, out$meanRisk$landmark, out$meanRisk$treatment, sep = "."),
      ...
      if (return.iid.nuisance && (attr(estimator, "IPTW") == TRUE)) {
        ...
        out$iid <- list(
          GFORMULA = pointEstimate$store$iid.GFORMULA,
...
ate_initArgs <- function(object.event,
  ...
  if (is.null(data.index)) {
    data.index <- seq_len(NROW(mydata))
  }
  ...
  } else if (is.list(object.event) && all(sapply(object.event, function(iE) {
    inherits(iE, "formula")
  }))) { ## list of formula
    formula.event <- object.event
  ...
  if (inherits(model.censor, "coxph") || inherits(model.censor, "cph") || inherits(model.censor, "phreg")) {
    ...
    n.censor <- sapply(times, function(t) {
      sum(test.censor * (censoringMF$stop <= t))
    })
    ...
    if (inherits(model.event, "CauseSpecificCox")) {
      ...
      n.censor <- sapply(times, function(t) {
        sum(test.censor * (model.event$response[, "time"] <= t))
      })
  ...
  if (inherits(model.event, "CauseSpecificCox")) {
      ...
      if (any(model.event$y[, NCOL(model.event$y)] == 1)) {
        ...
        if (inherits(x = modeldata, what = "try-error")) {
          if (NROW(mydata) == NROW(model.event$y)) {
            ...
            stop(
              "Cannot guess which value of the variable \"", eventVar.status, "\" corresponds to the outcome of interest \n",
    ...
    if (is.null(product.limit)) {
      product.limit <- (length(level.states) > 1)
    }
  ...
  n.obs <- c(
    data = NROW(mydata),
    model.event = if (!is.null(model.event)) {
      stats::nobs(model.event)
    } else {
      NA
    },
    model.treatment = if (!is.null(model.treatment)) {
      stats::nobs(model.treatment)
    } else {
      NA
    },
    model.censor = if (!is.null(model.censor)) {
      coxN(model.censor)
    } else {
      NA
    }
...
ate_checkArgs <- function(call,
  ...
  if (inherits(object.event, "glm") && length(times) != 1) {
    stop(
      "Argument \'times\' has no effect when using a glm object \n",
  ...
  if (any(estimator %in% valid.estimator == FALSE)) {
    stop(
      "Incorrect value(s) for argument \'estimator\': \"", paste(estimator[estimator %in% valid.estimator == FALSE], collapse = "\" \""), "\"\n",
  ...
  if (!is.null(object.event)) {
    ...
    if (inherits(object.event, "wglm") && (cause != object.event$theCause)) {
      stop(
        "Argument \'cause\' does not match the one of \'object.event\' \n",
  ...
  if (!is.null(object.treatment)) {
    if (!inherits(object.treatment, "multinom") && (!inherits(object.treatment, "glm") || object.treatment$family$family != "binomial")) {
      stop(
        "Argument \'treatment\' must be a logistic regression (glm object) or a multinomial regression (nnet::multinom)\n",
    ...
    if (any(levels(mydata[[treatment]]) %in% mydata[[treatment]] == FALSE)) {
      ...
      stop(
        "Argument \'mydata\' needs to take all possible treatment values when using IPTW/AIPTW \n",
    ...
    if (inherits(object.treatment, "glm") && object.treatment$family$family == "binomial" && length(unique(mydata[[treatment]])) > 2) {
      stop(
        "Cannot use a logistic regression in argument \"treatment\" when there are more than two treatment categories \n",
  ...
  if (attr(estimator, "IPCW")) {
    ...
    if (any(cause == level.censoring)) {
      stop(
        "The level indicating censoring should differ between the outcome model and the censoring model \n",
  ...
  if (B > 0) {
    ...
    if (iid == TRUE) {
      stop(
        "Influence function cannot be computed when using the bootstrap approach \n",
    ...
    if (band == TRUE) {
      stop(
        "Confidence bands cannot be computed when using the bootstrap approach \n",
  ...
  if (B == 0 && (se || band || iid)) {
    ...
    if (is.null(data.index) || (is.null(call$data.index) && test.data.index)) {
      stop(
        "Incompatible number of observations between argument \'data\' and the dataset from argument(s) \'", paste(names(stats::na.omit(n.obs[-1])), collapse = "\' "), "\' \n",
    ...
    if (!is.numeric(data.index) || any(duplicated(data.index)) || any(is.na(data.index)) || test.data.index) {
      stop(
        "Incorrect specification of argument \'data.index\' \n",
    ...
    if (!is.null(object.treatment) && stats::nobs(object.treatment) != NROW(mydata)) { ## note: only check that the datasets have the same size
      stop(
        "Argument \'treatment\' must be have been fitted using argument \'data\' for the functional delta method to work\n",
    ...
    if (!is.null(object.censor) && coxN(object.censor) != NROW(mydata)) { ## note: only check that the datasets have the same size
      stop(
        "Argument \'censor\' must be have been fitted using argument \'data\' for the functional delta method to work\n",
        "(discrepancy found in number of rows) \n"
      )
    }
  }
  ...
  if (!is.null(treatment)) {
    ...
    if (is.numeric(mydata[[treatment]])) {
      stop(
        "The treatment variable must be a factor variable. \n",
  ...
  if (!is.null(contrasts)) {
    if (any(contrasts %in% unique(mydata[[treatment]]) == FALSE)) {
      stop(
        "Incorrect values for the argument \'contrasts\' \n",
  ...
  if (!is.null(allContrasts)) {
    if (any(allContrasts %in% unique(mydata[[treatment]]) == FALSE)) {
      stop(
        "Incorrect values for the argument \'allContrasts\' \n",
        "Possible values: \"", paste(unique(mydata[[treatment]]), collapse = "\" \""), "\" \n"
      )
    }
  ...
  if (!is.na(eventVar.time)) {
    ...
    # freq.event <- tapply(data.status, data.strata, function(x) {
    #   mean(x == cause)
    # })
    count.event <- tapply(data.status, data.strata, function(x) {
      sum(x == cause)
    })
  ...
  if ((return.iid.nuisance == FALSE) && (iid == TRUE) && (attr(estimator, "augmented") == FALSE)) {
    warning(
      "Ignoring the uncertainty associated with the estimation of the nuisance parameters may lead to inconsistent standard errors. \n",
...
vcov.ate <- function(object, ...) {
  if (is.null(object$iid)) {
    stop(
      "Missing iid decomposition in object \n",
