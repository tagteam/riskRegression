...
#' #### no censoring ####
#' e.wglm <- wglm(
#'   regressor.event = ~X1, formula.censor = Surv(time, event == 0) ~ 1,
...
wglm <- function(regressor.event, formula.censor, times, data, cause = NA,
  ...
  if (any(newname %in% names(data))) {
    stop(
      "Argument \'data\' should not have a column named \"", paste0(newname[newname %in% names(data)], collapse = "\" \""), "\"\n",
      "This name is used internally \n."
    )
  }
  if (any(obs.newname %in% names(data))) {
    stop(
      "Argument \'data\' should not have a column named \"", paste0(obs.newname[obs.newname %in% names(data)], collapse = "\" \""), "\"\n",
      "This name is used internally \n."
    )
  }
  if (any(IPCW.newname %in% names(data))) {
    stop(
      "Argument \'data\' should not have a column named \"", paste0(IPCW.newname[IPCW.newname %in% names(data)], collapse = "\" \""), "\"\n",
      "This name is used internally \n."
    )
  }
  if (any(is.na(data))) {
  ...
  n.censor <- sapply(times, function(iTime) {
    sum((recoverTime < iTime) * recoverCensor)
  })
  ...
  for (iTime in 1:n.times) {
    ...
    if (n.censor[iTime] > 0) {
      iPred <- predictRisk(object.censor,
        diag = TRUE, newdata = data, times = pmin(recoverTime, times[iTime]) - tol,
        type = "survival", product.limit = product.limit
      )
      data[[IPCW.newname[[iTime]]]] <- ifelse(data[[obs.newname[iTime]]], 1 / iPred[, 1], 0)
      out$fit[[iTime]] <- suppressWarnings(do.call(stats::glm, list(
        formula = iFormula.glm, family = stats::binomial(link = "logit"),
      ...
      out$fit[[iTime]] <- do.call(stats::glm, list(
        formula = iFormula.glm,
...
coef.wglm <- function(object, times = NULL, simplifies = TRUE, ...) {
  if (is.null(times)) {
    ...
    if (any(times %in% object$times == FALSE)) {
      stop(
        "Incorrect specification of argument \'times\' \n",
...
iid.wglm <- function(x, times = NULL, simplifies = TRUE, ...) {
  ...
  for (iTime in 1:n.times) {
    ...
    if (x$n.censor[iTime] > 0) {
      ...
      factor <- TRUE
      attr(factor, "factor") <- lapply(apply(colMultiply_cpp(X, -W2 * (Y - pi)), 2, list), function(iVec) {
        cbind(iVec[[1]])
      })
      iPred <- predictRisk(x$cox,
        diag = TRUE, newdata = x$data, times = iObject$time.prior.weights,
...
predictRisk.wglm <- function(object, newdata, times = NULL,
  ...
  if (average.iid) {
    if (is.null(attr(average.iid, "factor"))) {
      ...
      for (iFactor in seq_len(length(factor))) { ## iFactor <- 1
  ...
  if (average.iid) {
    attr(out, "average.iid") <- lapply(1:n.factor, function(x) {
      matrix(NA, nrow = n.sample, ncol = n.times)
    })
  ...
  for (iTime in 1:n.times) {
    ...
    if (!is.null(level)) {
      matching.Ylevel <- table(
        iObject$data[[all.vars(formula(iObject))[1]]],
        iObject$y
      )
