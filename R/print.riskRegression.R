...
print.riskRegression <- function(x,
  ...
  cat("\nIPCW weights: ",
    switch(tolower(x$cens.model),
      ...
      "aalen" = {
        "non-parametric additive Aalen model"
      }
    ),
  ...
  if (is.null(x$timeConstantEffects$coef)) {
    ...
    if (length(Flevels) > 0) {
      fvars <- lapply(seq_len(length(Flevels)), function(i) {
        paste0(names(Flevels)[[i]], Flevels[[i]])
      })
    ...
    if (!is.null(coefMat)) {
      ...
      cat(paste("\n\nNote: The coefficients (Coef) are", switch(x$link,
        "prop" = "sub-hazard ratios (Fine & Gray 1999)",
        "logistic" = "odds ratios",
        "additive" = "absolute risk differences",
        "relative" = "absolute risk ratios"
