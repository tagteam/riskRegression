...
iidCox <- function(object, newdata = NULL, tauLambda = NULL, tauBeta = NULL, keep.times = TRUE, 
  ...
  # check if it is ok for interactions, categorical variables
  ...
  object.LPdata <- as.matrix(CoxData(object, data = model.frame(object), lpVars = infoVar$lpvars, stratavars = NULL, center = center.LPdata))
  ...
  if(is.null(newdata)){
    ...
    new.eXb <- exp(CoxLP(object, data = newdata, lpVars = infoVar$lpvars, stratavars = infoVar$stratavars, center = center.eXb))
    new.LPdata <- as.matrix(CoxData(object, data = newdata, lpVars = infoVar$lpvars, stratavars = infoVar$stratavars, center = center.LPdata))
  ...
  ICbeta <- iidBeta(tau = tauBeta, newT = new.time, neweXb = new.eXb, newX = new.LPdata, newStatus = new.status,
                    S01 = S0_U1times, E1 = E_U1times, indexTime1 = indexTime1, time1 = U1.time, 
                    n = object.n, p = p, iInfo = iInfo)
  if(keep.times){attr(ICbeta,"time") <- tauBeta}
  
  #### Influence function for the baseline hazard
  # this function is slow for the moment
  lambda0 <- predictCox(object, type = "hazard", centered = center.lambda0)
  ...
  ## rescale
  if(center.result == TRUE){
    ICLambda0 <- ICLambda0 * exp(-as.double(coef(object) %*% object$means))
  }
