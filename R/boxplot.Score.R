...
boxplot.Score <- function(x,
  ...
  if (missing(main)) {
    if (type == "risk") {
      ...
      main <- switch(x$response.type,
        "competing.risks" = {
          paste("Difference in predicted risks\nof an event of type", x$cause, "\nbefore time", timepoint)
        },
        "survival" = {
          paste("Difference in predicted risks\nof an event before time", timepoint)
        },
        "binary" = {
          "Difference in predicted risks"
        }
      )
    }
  }
  if (missing(outcome.label)) {
    outcome.label <- switch(x$response.type,
      "competing.risks" = {
        paste("Event status\nat time", timepoint)
      },
      "survival" = {
  ...
  ypos <- c((seq_len(length(causes))) - 0.5, length(causes))
  text(
    x = xlim[1],
  ...
  for (i in seq_len(length(causes))) {
    cc <- causes[[i]]
    args.i <- control$bxp
