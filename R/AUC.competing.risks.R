...
AUC.competing.risks <- function(DT,MC,se.fit,conservative,cens.model,keep.vcov=FALSE,multi.split.test,alpha,N,NT,NF,dolist,cause,states,ROC,IC.data,...){
    ...
    if (se.fit[[1]]==1L || multi.split.test[[1]]==TRUE){
      ...
      if (conservative[[1]]){
        aucDT[,IF.AUC:=getInfluenceCurve.AUC(times[1],time,status*event, WTi, Wt, risk, ID, NULL, nth.times[1]), by=list(model,times)]
      }
        ...
        aucDT[,IF.AUC:=getInfluenceCurve.AUC(times[1],time,status*event, WTi, Wt, risk, ID, MC, nth.times[1]), by=list(model,times)]
      }
      else {
