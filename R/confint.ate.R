...
confint.ate <- function(object,
  ...
  if (!is.null(allContrasts)) {
    ...
    if (any(interaction(allContrasts[1, ], allContrasts[2, ]) %in% interaction(object$allContrasts[1, ], object$allContrasts[2, ]) == FALSE)) {
      stop(
        "Incorrect combinations for the argument \'allContrasts\' \n",
  ...
  out <- list(
    meanRisk = data.table::copy(object$meanRisk[, .SD, .SDcols = setdiff(names(object$meanRisk), rm.col)]),
  ...
  if (ci) {
    ...
    out$diffRisk[, c("lower", "upper", if (p.value) {
      "p.value"
    }) := as.numeric(NA)]
    out$ratioRisk[, c("lower", "upper", if (p.value) {
      "p.value"
    }) := as.numeric(NA)]
  }
  if (band > 0) {
...
confintBoot.ate <- function(object, estimator, out, seed) {
  ...
  if (ci) {
    try.CI <- try(ls.CI <- lapply(index, function(iP) { # iP <- 1
      if (n.boot[iP] == 0) {
        ...
        return(c(
          lower = as.double(object$boot$t0[iP] + qnorm(alpha / 2) * boot.se[iP]),
          upper = as.double(object$boot$t0[iP] - qnorm(alpha / 2) * boot.se[iP])
        ))
      } else if (bootci.method == "quantile") {
        return(c(
          lower = as.double(quantile(object$boot$t[, iP], probs = alpha / 2, na.rm = TRUE)),
        ...
        if (requireNamespace("boot", quietly = TRUE)) {
          out <- boot::boot.ci(object$boot,
            ...
            type = bootci.method,
            index = iP
          )[[slot.boot.ci]][index.lowerCI:index.upperCI]
        } else {
  ...
  for (iE in seq_len(length(estimator))) { ## iE <- 1
...
confintIID.ate <- function(object, estimator, out, seed) {
  ...
  if (is.null(object$iid) || any(sapply(object$iid[estimator], is.null))) {
    stop(
      "Cannot re-compute standard error or confidence bands without the iid decomposition \n",
  ...
  ## ** run
  for (iE in seq_len(n.estimator)) { ## iE <- 1
    iEstimator <- estimator[iE]
    ## ** meanRisk
    ## cat("meanRisk \n")
    ...
    for (iC in seq_len(n.contrasts)) { ## iC <- 1
    ...
    ## compute
    CIBP.mR <- transformCIBP(
      estimate = estimate.mR,
      se = se.mR,
      iid = iid.mR,
      null = NA,
    ...
    for (iC in seq_len(n.contrasts)) { ## iC <- 2
    ...
    for (iC in seq_len(n.allContrasts)) { ## iC <- 1
    ...
    ## compute
    CIBP.dR <- transformCIBP(
      estimate = estimate.dR,
      se = se.dR,
      iid = iid.dR,
      null = 0,
      conf.level = conf.level,
    ...
    ## compute
    CIBP.rR <- transformCIBP(
      estimate = estimate.rR,
      se = se.rR,
      iid = iid.rR,
      null = 1,
      conf.level = conf.level,
