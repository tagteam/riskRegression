predictCoxWeights <- function(object,
  ...
  object.modelFrame[, c("XXXindexXXX") := seq_len(.N)] ## keep track of the initial positions (useful when calling calcSeCox)
  ...
  c(weightedAverageIFCumhazard_cpp(
    seqTau = times,
    ...
    diag = diag,
    debug = 0, weights = weights, isBeforeTau = isBeforeTau, tau = tau
  )) * new.n
}
