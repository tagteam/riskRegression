...
transformCIBP <- function(estimate, se, iid, null,
    ...
    if (method.band %in% c("maxT-integration", "maxT-simulation")) {
      ...
      if (band == 1) {
        if (n.time == 1) {
          ...
          rho <- lapply(1:n.contrast, function(iC) {
            tcrossprod(iid.norm[, , iC])
          })
        }
      } else if (band == 2) {
        if (n.time == 1) {
          ...
          rho <- tcrossprod(do.call(rbind, lapply(1:n.contrast, function(iC) {
            iid.norm[, , iC]
          })))
        }
      }
  ...
  if (ci) {
    if (alternative == "two.sided") {
      if (is.null(df)) {
        zval <- c(
          stats::qnorm((1 - conf.level) / 2, mean = 0, sd = 1),
      ...
      if (is.null(df)) {
        zval <- c(
          stats::qnorm(1 - conf.level, mean = 0, sd = 1),
          Inf
        )
      } else {
        zval <- c(
          stats::qt(1 - conf.level, df = df),
          Inf
        )
      }
  ...
  if (band > 0) {
    if (method.band == "bonferroni") {
      if (alternative == "two.sided") {
        quantileBand <- c(
          stats::qnorm((1 - conf.level) / (2 * n.test), mean = 0, sd = 1),
        ...
        quantileBand <- c(
          stats::qnorm((1 - conf.level) / n.test, mean = 0, sd = 1),
          Inf
        )
      }
      ...
      if (method.band == "maxT-integration") {
        if (band == 1) {
          for (iC in 1:n.contrast) { ## iC <- 1
            ...
            resQ <- mvtnorm::qmvnorm(
              p = conf.level, mean = rep(0, NCOL(rho[[iC]])),
          ...
          resQ <- mvtnorm::qmvnorm(
            p = conf.level, mean = rep(0, NCOL(rho)),
    ...
    if (method.band %in% c("bonferroni", "maxT-integration", "maxT-simulation")) {
      ...
      out[c("lowerBand", "upperBand")] <- transformCI(
        estimate = estimate,
        se = se,
    ...
    if (p.value) {
      ...
      if (method.band %in% p.adjust.methods) {
        if (band == 1) {
          for (iC in seq_len(NROW(out[["p.value"]]))) {
            ...
            out$adj.p.value[iC, index.keep] <- sapply(statistic[iC, index.keep], function(iT) { ## iT
              if (alternative == "two.sided") {
                return(1 - mvtnorm::pmvnorm(
                  lower = rep(-abs(iT), iN.test), upper = rep(abs(iT), iN.test),
                  mean = rep(0, iN.test), sigma = rho[[iC]]
                ))
              } else if (alternative == "greater") {
                return(1 - mvtnorm::pmvnorm(
                  lower = rep(-Inf, iN.test), upper = rep(iT, iN.test),
                  mean = rep(0, iN.test), sigma = rho[[iC]]
                ))
              } else if (alternative == "less") {
                return(1 - mvtnorm::pmvnorm(
                  lower = rep(iT, iN.test), upper = rep(Inf, iN.test),
                  mean = rep(0, iN.test), sigma = rho[[iC]]
                ))
              }
            })
          ...
          for (iC in 1:n.contrast) { ## iC <- 2
            ...
            out$adj.p.value[iC, index.keep] <- sapply(statistic[iC, index.keep], function(iT) {
              if (alternative == "two.sided") {
                ...
                return(1 - mvtnorm::pmvnorm(
                  lower = rep(-Inf, iN.test), upper = rep(iT, n.test),
                  mean = rep(0, iN.test), sigma = rho
                ))
              } else if (alternative == "less") {
                return(1 - mvtnorm::pmvnorm(
        ...
        out$adj.p.value[, index.keep] <- pProcess_cpp(
          nSample = n.sample,
          nContrast = n.contrast,
...
as.data.table.transformCIBP <- function(x, keep.rownames = FALSE, ...) {
  ...
  for (iEndpoint in 1:n.endpoint) { ## iEndpoint <- 1
    iDT <- as.data.table(lapply(x[nameRow.x], function(iE) {
      iE[iEndpoint, ]
    }))
