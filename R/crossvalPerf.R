...
crossvalPerf.loob.AUC <- function(times,mlevs,se.fit,response.type,NT,Response,cens.type,Weights,split.method,N,B,DT.B,data,dolist,alpha,byvars,mlabels,conservative,cens.model,cause) {
  # initializing output
  bfold <- fold <- AUC <- ReSpOnSe <- status <- riskRegression_ID <- model <- b <- risk <- casecontrol <- IF.AUC <- IF.AUC0 <- se <- IF.AUC.conservative <- se.conservative <- lower <- upper <- NF <- reference  <-  event <- status0 <- NULL
...
crossvalPerf.loob.Brier <- function(times,mlevs,se.fit,response.type,NT,Response,cens.type,Weights,split.method,N,B,DT.B,data,dolist,alpha,byvars,mlabels,ipa,ibs,keep.residuals,conservative,cens.model,response.dim,riskRegression_ID,cause){
    ...
    ## DT.B <- DT.B[,data.table::data.table(residuals=sum(residuals)),by=c(byvars,"riskRegression_ID")]
    ...
    if (se.fit==1L){
        ...
        if (cens.type[1]=="rightCensored" && !conservative){
            ...
            # merge with 
            DT.B <- data[,c(1:response.dim,match("riskRegression_ID",names(data))),with=FALSE][DT.B,,on="riskRegression_ID"]
...
crossvalPerf.loob <- function(m,times,mlevs,se.fit,response.type,NT,Response,cens.type,Weights,split.method,N,B,DT.B,data,dolist,alpha,byvars,mlabels,ipa,ibs,keep.residuals,conservative,cens.model,response.dim,riskRegression_ID,cause) {
  if (m=="AUC"){
  ...
  if (m=="Brier"){
    return(crossvalPerf.loob.Brier(times,mlevs,se.fit,response.type,NT,Response,cens.type,Weights,split.method,N,B,DT.B,data,dolist,alpha,byvars,mlabels,ipa,ibs,keep.residuals,conservative,cens.model,response.dim,riskRegression_ID,cause))
  }
}
