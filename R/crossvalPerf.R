...
crossvalPerf.loob.AUC <- function(times,mlevs,se.fit,response.type,NT,Response,cens.type,Weights,split.method,N,B,DT.B,data,dolist,alpha,byvars,mlabels,conservative,cens.model) {
  ...
  for (s in 1:NT){
    ...
    for (mod in mlevs){
      ...
      w.list <- foreach::foreach(k = 1:N) %do% {
        matrix(0, sum(cases.index), sum(controls.index))
      }
      ...
      if (se.fit==1L){
        ...
        if (response.type != "binary"){
          if (response.type == "competing.risks"){
            status0 <- data[["event"]]*data[["status"]]
          }
          else {
            status0 <- data[["status"]]
          }
        ...
        if (is.null(t)){
          this.aucDT <- data.table(model=mod,ID = c(id.cases,id.controls))
        }else{
