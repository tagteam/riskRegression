...
crossvalPerf.loob.AUC <- function(times, mlevs, se.fit, response.type, NT, Response, cens.type, Weights, split.method, N, B, DT.B, data, dolist, alpha, byvars, mlabels, conservative, cens.model, cause) {
  # initializing output
  bfold <- fold <- AUC <- ReSpOnSe <- ID <- model <- b <- risk <- IF.AUC <- IF.AUC0 <- se <- lower <- upper <- NF <- reference <- status0 <- NULL
      ...
      if (se.fit == 1L) { ## should clean this up when cv has been fixed !
        if (mod == 0) {
          ...
          if (!response.type == "binary" && !cens.type == "uncensored" && !conservative[[1]]) {
            ...
            icPart <- getInfluenceFunction.AUC.censoring.term(
              time = Response[["time"]],
  ...
  if (se.fit == 1L) {
    ...
    if (cens.type[1] == "rightCensored" && !conservative) {
      ...
      if (cens.type == "uncensored") {
        ...
        DT.B[, IF.Brier := getInfluenceCurve.Brier(
          t = times[1],
          time = time,
          IC0,
          residuals = residuals,
...
crossvalPerf.bootcv <- function(m, crossval, se.fit, multi.split.test, keep.cv, byvars, alpha) {
  ...
  if (length(crossval[[1]][[m]]$score) > 0) {
    cv.score <- data.table::rbindlist(lapply(crossval, function(x) {
      x[[m]]$score
    }))
    if (se.fit == TRUE) {
      bootcv.score <- cv.score[, data.table::data.table(mean(.SD[[m]], na.rm = TRUE),
        ...
        upper = quantile(.SD[[m]], (1 - alpha / 2), na.rm = TRUE)
      ), by = byvars, .SDcols = m]
  ...
  if (length(crossval[[1]][[m]]$contrasts) > 0) {
    cv.contrasts <- data.table::rbindlist(lapply(crossval, function(x) {
      x[[m]]$contrasts
    }))
    ...
    bootcv.contrasts <- switch(as.character(se.fit + 3 * multi.split.test),
      "4" = {
        cv.contrasts[, data.table::data.table(mean(.SD[[delta.m]], na.rm = TRUE),
          ...
          p = median(.SD[["p"]], na.rm = TRUE)
        ), by = c(byvars, "reference"), .SDcols = c(delta.m, "p")]
      },
      "1" = {
        cv.contrasts[, data.table::data.table(mean(.SD[[delta.m]], na.rm = TRUE),
          lower = quantile(.SD[[delta.m]], alpha / 2, na.rm = TRUE),
          upper = quantile(.SD[[delta.m]], (1 - alpha / 2), na.rm = TRUE)
        ), by = c(byvars, "reference"), .SDcols = c(delta.m)]
      },
      "3" = {
        cv.contrasts[, data.table::data.table(mean(.SD[[delta.m]], na.rm = TRUE),
          p = median(.SD[["p"]], na.rm = TRUE)
