...
#' averageCIF_CIsup: the lower bound of the CI for averageCIF
#' averageCIF_CIinf: the upper bound of the CI for averageCIF
#' gFormula_CIsup: the lower bound of the CI for averageCIF_CIsup
#' gFormula_CIinf: the upper bound of the CI for averageCIF_CIinf
#' 
#' @examples 
#' #library(snowfall)
...
#'        " [",
#'         res.Gformula$averageCIF_CIinf[res.Gformula$averageCIF_CIinf[,"time"] == 5,"T0"],
#'         " ; ",
#'         res.Gformula$averageCIF_CIsup[res.Gformula$averageCIF_CIsup[,"time"] == 5,"T0"],
#'         "]")
#' @export
#' 
ateCSC <- function(data, treatVar, treatLevels = NULL,
  ...
  if(n.bootstrap>0){
    ...
    if(ncpus > parallel::detectCores()){
      warning("Gformula: not enought available cores \n",
    ...
    res.boot <- sfClusterApplyLB(1:n.bootstrap, function(iterN){
    ...
    res.export$averageCIF_CIsup <- apply(simplify2array(lapply(res.boot, function(x){x[["averageCIF"]]})),
                                         1:2,
                                         quantile, prob = c(1-alpha/2)
    )
    res.export$averageCIF_CIinf <- apply(simplify2array(lapply(res.boot, function(x){x[["averageCIF"]]})),
                                         1:2,
                                         quantile, prob = c(alpha/2)
    )
    res.export$gFormula_CIsup <- apply(simplify2array(lapply(res.boot, function(x){x[["gFormula"]]})),
                                       1:3,
                                       quantile, prob = c(1-alpha/2)
    )
    res.export$gFormula_CIinf <- apply(simplify2array(lapply(res.boot, function(x){x[["gFormula"]]})),
                                       1:3,
                                       quantile, prob = c(alpha/2)
    )
  ...
  if(addTime){
    ...
    res.export$averageCIF_CIsup <- cbind(time = times, res.export$averageCIF_CIsup)
    res.export$averageCIF_CIinf <- cbind(time = times, res.export$averageCIF_CIinf)
    
    dimnames(res.export$gFormula)[[3]] <- times
    dimnames(res.export$gFormula_CIsup)[[3]] <- times
    dimnames(res.export$gFormula_CIinf)[[3]] <- times
  }
