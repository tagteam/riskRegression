...
IPA.CauseSpecificCox <- function(object, formula, newdata, times, cause, ...) {
  ...
  scope.list <- lapply(models, function(x) {
    stats::drop.scope(x)
  })
  scope <- unique(unlist(scope.list, use.names = 0))
  ## ns <- length(scope)
  n0 <- lapply(models, function(x) {
    stats::nobs(x, use.fallback = TRUE)
  })
  leaveOneOut <- lapply(scope, function(var) {
    varfit <- object
    varfit$models <- lapply(seq_len(length(models)), function(m) {
  ...
  if (missing(cause)) {
    cause <- attr(object$response, "states")[[1]]
  } else {
