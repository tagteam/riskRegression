...
Score.list <- function(object,
                       ...
                       cutpoints = NULL,
                       ...) {
  Brier <- AUC <- time <- status <- ID <- risk <- crossval <- b <- time <- status <- model <- p <- model <- pseudovalue <- ReSpOnSe <- event <- j <- NULL
...
c.f., Chapter 7, Section 5 in Gerds & Kattan 2021. Medical risk prediction models. ")
    ...
    switch(parallel,
      ...
      "multicore" = {
        if (.Platform$OS.type == "windows") {
          stop("multicore does not work on windows. set argument parallel to 'snow'.")
        }
  ...
  if (!missing(predictRisk.args)) {
    if (!(all(names(predictRisk.args) %in% unlist(object.classes)))) {
      stop(paste0(
        "Argument predictRisk.args should be a list whose names match the S3-classes of the argument object.
  ...
  if (do.resample) {
    lapply(seq_len(length(object)), function(f) {
      fit <- object[[f]]
  ...
  if ((NF + length(nullobject)) <= 1) {
    ...
    if ((is.logical(contrasts) && contrasts[1] == FALSE)) {
      ...
      if (is.logical(contrasts) && contrasts[1] == TRUE) {
        if (is.null(nullobject)) {
          dolist <- lapply(1:(NF - 1), function(i) {
            c(i, (i + 1):NF)
          })
        } else {
          dolist <- lapply(0:(NF - 1), function(i) {
            c(i, (i + 1):NF)
          })
        }
      } else {
  ...
  if (split.method$internal.name %in% c("BootCv", "LeaveOneOutBoot", "crossval")) {
    ...
    if (split.method$internal.name == "crossval" && B == 1) {
      crossvalPerf <- computePerformance(
        DT = DT.B,
        N = N,
        NT = NT,
        NF = NF,
      ...
      message(paste0(
        "Calculating the performance metrics in long format\nlevel-1 data with ",
        NROW(DT.B),
        " rows.",
    ...
    if ("risks" %in% summary) {
      crossvalPerf[["risks"]]$score <- DT.B[,
        {
          c(
            .SD[1, Response.names, with = FALSE],
            list(
              risk = mean(risk),
              sd.risk = sd(risk),
              oob = .N
  ...
  if (keep.vcov) {
    ...
    if (null.model) {
      ...
      names(lab.models) <- paste0("model=", seq_len(length(mlabels)))
    }
    ...
    if (!is.null(output$AUC$vcov)) {
      if (response.type == "binary") {
        ...
        for (ml in seq_len(length(mlabels))) {
