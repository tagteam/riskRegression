...
calcSeCSC <- function(object, cif, hazard, cumhazard, survival, object.time, object.maxtime,
  ...
  if (is.iidCox(object)) {
    for (iModel in 1:nCause) { # iModel <- 1
      if (!identical(object$models[[iModel]]$iid$time, object.time)) {
        object$models[[iModel]]$iid <- selectJump(object$models[[iModel]]$iid,
          times = object.time,
    ...
    object <- iidCox(object,
      tau.hazard = object.time,
  ...
  if (store.iid == "minimal") {
    ...
    resCpp <- calcSeMinimalCSC_cpp(
      ...
      newX = new.LPdata,
      neweXb = eXb,
      IFbeta = lapply(object$models, function(x) {
        x$iid$IFbeta
      }),
      Ehazard0 = object$models[[cause]]$iid$calcIFhazard$Elambda0,
      cumEhazard0 = lapply(object$models, function(x) {
        x$iid$calcIFhazard$cumElambda0
      }),
      hazard_iS0 = object$models[[cause]]$iid$calcIFhazard$lambda0_iS0,
      cumhazard_iS0 = lapply(object$models, function(x) {
        x$iid$calcIFhazard$cumLambda0_iS0
      }),
      delta_iS0 = lapply(object$models, function(x) {
        x$iid$calcIFhazard$delta_iS0
      }),
      sample_eXb = lapply(object$models, function(x) {
        x$iid$calcIFhazard$eXb
      }),
      sample_time = object$models[[cause]]$iid$obstime,
      indexJumpSample_time = lapply(object$model, function(x) {
        lapply(x$iid$calcIFhazard$time1, function(y) {
          prodlim::sindex(y, eval.times = object$model[[cause]]$iid$obstime) - 1
        })
      }),
      jump_time = object.time,
      isJump_time1 = do.call(cbind, lapply(object$model[[cause]]$iid$calcIFhazard$time1, function(x) {
        object.time %in% x
      })),
      jump2jump = lapply(object$model, function(x) {
        lapply(x$iid$calcIFhazard$time1, function(y) {
      ...
      lastSampleTime = apply(grid.strata, 1, function(iStrata) {
        min(sapply(1:nCause, function(iCause) {
          object$models[[iCause]]$iid$etime.max[iStrata[iCause] + 1]
        }))
      }),
    ...
    if ("average.iid" %in% export) {
      if (rm.list) {
        ...
        out$average.iid <- lapply(resCpp$IFmean_cif, function(iVec) {
          matrix(iVec, nrow = object.n, ncol = diag + (1 - diag) * nTimes)
        })
      }
    }
    # }}}
    ...
    if ("iid" %in% export || "se" %in% export) {
      out <- calcSeCif2_cpp(
        ls_IFbeta = lapply(object$models, function(x) {
          x$iid$IFbeta
        }),
        ls_X = new.LPdata,
        ...
        survival = survival,
        cif = cif,
        ls_IFcumhazard = lapply(object$models, function(x) {
          x$iid$IFcumhazard
        }),
      ...
      for (iStrata in 1:new.n.strata) { ## iStrata <- 1
        ...
        if (diag && length(iNA.times) > 0) {
          out <- lapply(out, function(iF) {
            matrix(NA, nrow = nrow(iF), ncol = ncol(iF))
          })
          break
        }
        ...
        iIFCumhazard0[all.cause] <- lapply(all.cause, function(iC) {
          subsetIndex(object$models[[iC]]$iid$IFcumhazard[[iStrata_causes[iC]]],
        ...
        iX <- lapply(1:nCause, function(iC) {
          new.LPdata[[iC]][iIndex_obs, , drop = FALSE]
        })

        ## linear predictor
        ieXb <- lapply(1:nCause, function(iC) {
          eXb[iIndex_obs, iC]
        })
        ...
        eXb1_S_eXbj[all.cause] <- lapply(all.cause, function(iC) {
          colMultiply_cpp(eXb1_S, ieXb[[iC]])
        })

        if (nVar.lp[cause] > 0) {
          eXb1_S_X1 <- lapply(1:nVar.lp[cause], function(iP) {
            colMultiply_cpp(eXb1_S, scale = iX[[cause]][, iP])
          })
        } else {
          eXb1_S_X1 <- NULL
        }
        ...
        for (iCause in all.cause) {
          if (nVar.lp[iCause] > 0) {
            eXb1_S_Xj_eXbj[[iCause]] <- lapply(1:nVar.lp[iCause], function(iP) {
              colMultiply_cpp(eXb1_S_eXbj[[iCause]], scale = iX[[iCause]][, iP])
            })
          }
        }
        ...
        for (iFactor in 1:n.factor) {
          ...
          if (is.null(factor[[iFactor]])) {
            ...
            if (diag) {
              ...
              iMfactor <- do.call(rbind, lapply(iSindexV.times, function(i) {
                c(rep(1, i), rep(0, iN.jump - i))
              }))
          ...
          for (iFactor2 in 1:n.factor2) { ## iFactor2 <- 1
            ...
            if (any(stats::na.omit(as.double(cif)) >= 1)) { ## average influence function only among datapoint with cif<1
              test.cif1 <- cif >= 1
              vec.index1 <- apply(test.cif1, MARGIN = 1, FUN = function(iRow) {
                which(iRow)[1]
              })
              ...
              for (iP in 1:n.pattern) { ## iP <- 1
                ...
                if (length(iIndex.keep) == 0) {
                  ...
                  iAIF[, iIndex.rangePattern] <- calcAICcif_R(
                    hazard0_cause = ihazard0_cause,
                    cumhazard0 = iCumhazard0,
                    IFhazard0_cause = iIFhazard0_cause,
                    IFcumhazard0 = iIFCumhazard0,
                    ...
                    eXb1_S_eXbj = lapply(eXb1_S_eXbj, function(iM) {
                      iM[iIndex.keep, , drop = FALSE]
                    }),
                    eXb1_S_X1 = lapply(eXb1_S_X1, function(iM) {
                      iM[iIndex.keep, , drop = FALSE]
                    }),
                    eXb1_S_Xj_eXbj = lapply(eXb1_S_Xj_eXbj, function(iLs) {
                      lapply(iLs, function(iM) {
                        iM[iIndex.keep, , drop = FALSE]
                      })
                    }),
                    ...
                    nCause = nCause, test_allCause = test_allCause, test_theCause = test_theCause,
                    nVar = nVar.lp
                  )[, iIndex.rangePattern, drop = FALSE]
                }
              }
            }
