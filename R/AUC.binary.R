...
AUC.binary <- function(DT,
  ...
  delongtest <- function(risk,
                         ...
                         se.fit,
                         keep.vcov) {
    lower <- upper <- p <- AUC <- se <- lower <- upper <- NULL
    if (keep.iid == TRUE) {
      warning("Argument 'keep.iid' is ignored. This function does not explicitely calculate the estimated influence function for AUC with binary outcome.")
    }
    auc <- score[["AUC"]]
    nauc <- ncol(risk)
  ...
  if (ROC == FALSE) {
    ...
    if (!is.null(cutpoints)) {
      ...
      res.cut <- list()
      for (i in seq_len(nrow(length(cutpoints)))) {
