...
#' predict(CSC.fit,
#'   newdata = nd, times = 1:10, cause = 2, se = TRUE,
#'   keep.newdata = FALSE
#' )
#'
#' ## other example
#' library(survival)
#' CSC.fit.s <- CSC(list(
#'   Hist(time, event) ~ strata(X1) + X2 + X9,
#'   Hist(time, event) ~ X2 + strata(X4) + X8 + X7
...
predict.CauseSpecificCox <- function(object,
  ...
  if (type == "survival" && object$surv.type == "survival") {
    predictor.cox <- if (product.limit > 0) {
      "predictCoxPL"
    } else {
      "predictCox"
    }
    return(do.call(predictor.cox,
      args = list(object$models[["OverallSurvival"]],
        times = times, newdata = newdata, type = "survival",
  ...
  if (surv.type == "survival") {
    if (object$theCause != cause) {
      stop(
        "Object can be used to predict cause ", object$theCause,
  ...
  if (average.iid == TRUE && !is.null(attr(average.iid, "factor"))) {
    ...
    for (iFactor in seq_len(length(attr(average.iid, "factor")))) { ## iFactor <- 1
      ## check dimensions
  ...
  needOrder <- !identical(seq_len(length(ootimes)), ootimes)
  ...
  for (iterC in 1:nModel) { ## iterC <- 1
    ...
    strataTempo <- coxStrata(object$models[[iterC]],
      data = newdata,
  ...
  if (type == "absRisk") {
    ...
    return(.predictSurv_CSC(object,
      times = times, newdata = newdata, ls.hazard = ls.hazard, eXb = M.eXb,
  ...
  if (se || band || iid || average.iid) {
    ...
    needOrder2 <- !identical(seq_len(length(ootimes2)), ootimes2)
  }
  ...
  if (average.iid) {
    if (needOrder2 && (diag == FALSE)) {
      if (is.list(out.seCSC$average.iid)) {
        out$absRisk.average.iid <- lapply(out.seCSC$average.iid, function(iIID) {
          iIID[, ootimes2, drop = FALSE]
        })
      } else {
  ...
  if (keep.strata == TRUE) {
    ...
    if (length(allStrata) > 0) {
      ...
      newdata[, (allStrata) := lapply(allStrata, function(col) {
        paste0(col, "=", .SD[[col]])
      })]
...
.predictSurv_CSC <- function(object, times, newdata, type, ls.hazard, eXb, etimes, strata, etimeMax,
  ...
  out <- list(
    survival = NULL, survival.iid = NULL, survival.average.iid = NULL,
    lastEventTime = NA,
  ...
  for (iObs in 1:new.n) { ## iObs <- 1
    if (diag) {
      if (times[iObs] > etimeMax[iObs]) {
        ...
        if (product.limit) {
          ihazard <- lapply(1:nCause, function(iC) {
            ls.hazard[[iC]][1:index.times[iObs], strata[iObs, iC] + 1] * eXb[iObs, iC]
          })
          out$survival[iObs, 1] <- prod(1 - rowSums(do.call(cbind, ihazard)))
        } else {
          iCumhazard <- sapply(1:nCause, function(iC) {
            ls.cumhazard[[iC]][iObs, strata[iObs, iC] + 1] * eXb[iObs, iC]
          })
      ...
      if (product.limit) {
        ihazard <- lapply(1:nCause, function(iC) {
          ls.hazard[[iC]][, strata[iObs, iC] + 1] * eXb[iObs, iC]
        })
        ...
        iCumhazard <- lapply(1:nCause, function(iC) {
          ls.cumhazard[[iC]][, strata[iObs, iC] + 1] * eXb[iObs, iC]
        })
  ...
  if (iid || se || band || average.iid) {
    ...
    if (average.iid) {
      if (is.null(factor)) {
        ...
        iAverageIid <- lapply(1:n.factor, function(iF) {
          matrix(0, nrow = n.sample, ncol = n.times2)
        })
      }
    }
