...
print.ate <- function(x, estimator = x$estimator, ...) {
  ...
  cat(" - Estimator", if (length(estimator.print) > 1) {
    "s"
  } else {
    " "
  }, " : ", paste(estimator.print, collapse = " "), "\n", sep = "")
  if (test.inference) {
  ...
  if (display.results) {
    ...
    if (all(x$diffRisk[, .N, by = c("A", "B")]$N == 1)) {
      ...
      dt.res <- cbind(
        x$diffRisk[, list(
          risk.A = Publish::formatCI(lower = min(.SD$estimate.A), upper = max(.SD$estimate.A)),
