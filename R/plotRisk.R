...
plotRisk <- function(x,
                     ...
                     ...) {
  model <- ReSpOnSe <- status <- event <- cause <- NULL
  ...
  if (missing(models)) {
    ...
    if (!missing(models)) {
      ...
      if (is.numeric(models)) {
        if (any(!(models %in% fitted.models))) {
        ...
        if (any(!(models %in% names(fitted.models)))) {
          stop(paste("Cannot find all models. Fitted models are:\n", paste(paste0(x$models, ": ", names(x$models)), collapse = "\n")))
        }
      }
    }
    ...
    if (missing(col)) {
      colcode <- switch(x$response.type,
        ...
        "competing.risks" = {
          c("#52da58", "gray55", "red", rep("purple", nCR))
        }
      )
    } else {
      if (nR != length(col)) {
    ...
    this.legend <- paste0(levels(Rfactor), " (n=", sapply(levels(Rfactor), function(x) {
      sum(Rfactor == x)
    }), ")")
    if (x$response.type == "survival") {
      message(
        "Counts of events and censored are evaluated at the prediction time horizon (times=", times, "):\n",
