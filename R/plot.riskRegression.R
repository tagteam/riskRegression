...
plot.riskRegression <- function(x,
  ...
  if (inherits(x = x, what = "predictedRisk")) {
    Y <- split(x$risk, seq_len(NROW(x$risk)))
  } else {
    if (missing(newdata)) {
      ...
      if (NROW(xdat) < 5) {
        ...
        rownames(p1) <- paste("id", seq_len(NROW(xdat)))
      } else {
        if ("CauseSpecificCox" %in% class(x)) {
        ...
        p1 <- P1[round(quantile(seq_len(NROW(P1)))), ]
    ...
    Y <- lapply(seq_len(NROW(p1)), function(i) {
      p1[i, ]
    })
