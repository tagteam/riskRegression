cumincCox <- function (formula,data,cause,survtype="hazard",...){
  # {{{ type
  ...
  # }}}
  out <- list(call=call,models=CoxModels,response=response,eventTimes=eventTimes,survtype=survtype,theCause=theCause,causes=causes)
  class(out) <- "cumincCox"
  out
}
