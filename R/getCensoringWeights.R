    ...
    "cox" = {
      ...
      if (influence.curve == TRUE) {
        ...
        if (!censoring.save.memory) {
          ...
          ##                           IC_G(t,z;x_k)
          IC.subject <- predictCox(fit,
            iid = TRUE,
            newdata = wdata,
            times = TiMinus,
            diag = TRUE,
            type = "cumhazard"
          )$cumhazard.iid * NROW(data) ## we want the influence function   survival.iid*n
          IC.times <- predictCox(fit,
            iid = TRUE,
            newdata = wdata,
            times = times,
            type = "cumhazard"
          )$cumhazard.iid * NROW(data)
          IC.weights <- list()
          for (t.ind in seq_len(length(times))) {
            N <- length(Y)
