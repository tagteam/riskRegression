...
#' ggplot2::autoplot(ateFit,
#'   plot.type = "2", smooth = TRUE,
#'   band = FALSE, type = "diffRisk"
#' )
#' ggplot2::autoplot(ateFit,
#'   plot.type = "2", smooth = TRUE, first.derivative = TRUE,
#'   band = FALSE, type = "diffRisk"
#' )
#' }
## * autoplot.ate (code)
...
autoplot.ate <- function(object,
  ...
  if (plot.type == "1") {
    ...
    if (band > 0) {
      if (test.ylab) {
        if (band == 1) {
          ylab <- paste0(ylab, " (simultaneous ci over time)")
        }
        if (band == 2) {
          ylab <- paste0(ylab, " (simultaneous ci over time and treatment)")
        }
      }
