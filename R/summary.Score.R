...
summary.Score <- function(object,
                          ...
                          ...) {
  ## score
  out <- AUC <- model <- lower <- upper <- Brier <- Model <- reference <- delta.AUC <- p <- delta.Brier <- Reference <- NULL
  what <- sapply(tolower(what), function(w) {
    match.arg(w, c("score", "contrasts"))
  })
  # check models
  ...
  if (object$response.type == "binary") {
    ...
    if ("score" %in% tolower(what)) {
      if ("upper" %in% names(object$AUC$score)) {
        tab.AUC <- object$AUC$score[
          (model %in% models),
          data.table::data.table(
            Model = factor(as.character(model), levels = fitted.models)[, drop = TRUE],
