...
ATE_TI <- function(object.event,
  ...
  if (attr(estimator, "export.GFORMULA")) {
    ...
    out$store$iid.GFORMULA <- lapply(1:n.contrasts, function(iC) {
  ...
  if (attr(estimator, "export.IPTW")) {
    ...
    out$store$iid.IPTW <- lapply(1:n.contrasts, function(iC) {
      matrix(0, nrow = n.obs, ncol = n.times)
    })
  ...
  if (attr(estimator, "export.AIPTW")) {
    ...
    out$store$iid.AIPTW <- lapply(1:n.contrasts, function(iC) {
      matrix(0, nrow = n.obs, ncol = n.times)
    })
  ...
  if (attr(estimator, "IPTW")) {
    ...
    if (inherits(object.event, "glm") || (is.null(object.event) && is.null(object.censor))) {
      ...
      time.before.tau <- sapply(times, function(tau) {
        mydata[[eventVar.time]] <= tau
      })
    }
  ...
  if (attr(estimator, "integral")) {
    ...
    beforeEvent.jumpC <- do.call(cbind, lapply(time.jumpC, function(iJump) {
      iJump <= mydata[[eventVar.time]]
    }))
    beforeTau.nJumpC <- sapply(times, function(iTau) {
      sum(time.jumpC <= iTau)
    })
  ...
  ## *** treatment model
  if (attr(estimator, "IPTW")) {
    iPred <- lapply(contrasts, function(iC) {
      predictRisk(object = object.treatment, newdata = mydata, levels = iC, iid = (method.iid == 2) * return.iid.nuisance)
    })
  ...
  if (attr(estimator, "IPCW")) {
    iPred <- lapply(1:n.times, function(iT) {
      1 - predictRisk(object.censor,
        newdata = mydata, times = c(0, time.jumpC)[index.obsSINDEXjumpC[, iT] + 1],
    ...
    if (return.iid.nuisance && (method.iid == 2)) {
      out$store$iid.nuisance.censoring.diag <- lapply(iPred, function(x) {
        -attr(x, "iid")
      })
    }
  ...
  if (attr(estimator, "GFORMULA")) {
    ...
    for (iC in 1:n.contrasts) {
      ...
      outRisk <- predictRisk(object.event,
        newdata = data.i, times = times,
  ...
  if (attr(estimator, "integral")) {
    ...
    predTempo <- predictRisk(object.event,
      newdata = mydata, times = c(times, time.jumpC), cause = cause, product.limit = product.limit,
    ...
    S.jump <- predictRisk(object.event,
      type = "survival", newdata = mydata, times = time.jumpC - tol, product.limit = product.limit,
      iid = (method.iid == 2) * return.iid.nuisance
    )
    if ((method.iid == 2) * return.iid.nuisance) {
    ...
    G.jump <- 1 - predictRisk(object.censor,
      newdata = mydata, times = if (index.lastjumpC > 1) {
        c(0, time.jumpC[1:(index.lastjumpC - 1)])
      } else {
        0
      },
    ...
    dN.jump <- do.call(rbind, lapply(1:n.obs, function(iObs) {
      (mydata[[eventVar.time]][iObs] == time.jumpC) * (mydata[[eventVar.status]][iObs] == level.censoring)
    }))
...
ATE_COMPARISONS <- function(data, TD, allContrasts) {
  ...
  mdata <- do.call(rbind, apply(allContrasts, 2, function(iC) {
    merge(dataA[iC[1], .SD, on = "A"], dataB[iC[2], .SD, on = "B"],
      by = c("estimator", "time", if (TD) {
        "landmark"
      })
    )
  })) ## iC <- c("T0","T1")
  ...
  ## compute stats
  out <- list(
    diffRisk = data.table::copy(mdata),
