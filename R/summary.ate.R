...
summary.ate <- function(object, estimator = object$estimator[1], short = FALSE,
  ...
  ## ** update confint
  args.confint <- c(
    "conf.level", "n.sim", "contrasts", "allContrasts", "meanRisk.transform", "diffRisk.transform", "ratioRisk.transform", "seed",
    ...
    if ("diffRisk" %in% type) {
      ...
      if (short == 0) {
        ...
        if (!is.na(object$variable["strata"])) {
          ...
          cat(
            " (ratio of average risks when treating all subjects with the experimental treatment (B),\n",
            "                          vs. treating all subjects with the reference treatment (A))\n"
          )
        }
        cat("\n")
