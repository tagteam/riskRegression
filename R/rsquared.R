...
rsquared.CauseSpecificCox <- function(object,formula,newdata,times,cause,...){
    IPA=model=Variable=IPA.loss=Brier=NULL
    ...
    r2[,IPA.loss:=IPA[Variable=="Full model"]-IPA,by=times]
    ## r2 <- r2[Variable!="Full model"]
    out <- as.data.frame(r2[,data.table::data.table(Variable,times,Brier,IPA,IPA.loss)])
    class(out) <- c("IPA",class(out))
    out
}
...
rsquared.glm <- function(object,formula,newdata,...){
    ...
    ## r2 <- r2[model!="Null model"]
    data.table::setnames(r2,"model","Variable")
    r2[,IPA.loss:=IPA[Variable=="Full model"]-IPA]
    ## r2 <- r2[Variable!="Full model"]
    out <- as.data.frame(r2[,data.table::data.table(Variable,Brier,IPA,IPA.loss)])
    class(out) <- c("IPA",class(out))
    out
} 
