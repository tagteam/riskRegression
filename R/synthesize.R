...
synthesize.lvm <- function(object,
  ...
  if (!all(others <- names(data) %in% dimnames(object$M)[[1]])) {
    if (verbose) {
      warning(
        "Some variables in dataset are not in object (or the names don't match).\n These variables are not synthesized:\n",
  ...
  ismissingvar <- sapply(var.model, function(x) {
    anyNA(data[[x]])
  })
  if (any(ismissingvar)) {
  ...
  for (var in exogenous(object)) {
    ...
    if ("gaussian" %in% attributes(object$attributes$distribution[[var]])$family) {
      lava::distribution(sim_model, var_formula) <- lava::normal.lvm(
        mean = mean(data[[var]]),
        sd = sd(data[[var]])
      )
    }
  ...
  for (var in colnames(data)[grepl("factor|character", sapply(data, class))]) {
    ...
    if (!any(grepl(var, dimnames(object$M)[[1]]))) {
      ...
      for (lvl in levels(data[[var]])[-1]) {
        ...
        sim_model <- local({
          l <- lvl
          lava::transform(sim_model, formula) <- function(x) {
            1 * (x == l)
          }
          return(sim_model)
        })
  ...
  if (has.eventTime) {
    ...
    for (latvar in object$attributes$eventHistory[[timename]]$latentTime) {
      ...
      if (!(inherits(status_ind, "numeric"))) {
        stop("event or status variable has to be numeric")
      }
  ...
  for (v in logtrans) {
    transform(sim_model, as.formula(paste0(v, "~log", v))) <- function(x) {
      exp(x)
    }
  }
